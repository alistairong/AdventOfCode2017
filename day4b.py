myVar = """oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
myVar = [list(map(str, row.split('\t'))) for row in myVar.split('\n')]
...
def helper(elem):
	i = 0
	my_elem = sorted((''.join(str(e) for e in elem)).split(), key=str.lower)
	my_elem = list(map(arrange, my_elem))
	my_elem.sort()
	while i < len(my_elem) - 1:
		if my_elem[i] == my_elem[i + 1]:
			return False
		else:
			i += 1
	return True
